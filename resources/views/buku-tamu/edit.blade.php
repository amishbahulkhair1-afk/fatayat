    ...
    <div class="py-6 max-w-2xl mx-auto">
        <form action="{{ route('buku-tamu.update', $bukuTamu->id) }}" method="POST"
            ...
            <div>
                ...
                <input type="text" name="nama_tamu" value="{{ old('nama_tamu', $bukuTamu->nama_tamu) }}"
            ...
            <div>
                ...
                <input type="text" name="asal_instansi" value="{{ old('asal_instansi', $bukuTamu->asal_instansi) }}"
                    class="w-full border rounded p-2">
            </div>
            ...
            <div>
                ...
                <input type="text" name="tujuan_kunjungan"
                    value="{{ old('tujuan_kunjungan', $bukuTamu->tujuan_kunjungan) }}"
                    class="w-full border rounded p-2">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="tanggal_kunjungan"
                        value="{{ old('tanggal_kunjungan', $bukuTamu->tanggal_kunjungan) }}"
                        class="w-full border rounded p-2">
                ...
                <div>
                    ...
                    <input type="time" name="jam_kunjungan"
                        value="{{ old('jam_kunjungan', $bukuTamu->jam_kunjungan) }}" class="w-full border rounded p-2">
                </div>
            </div>

            <div>
                <label class="block font-medium">Kontak</label>
                <input type="text" name="kontak" value="{{ old('kontak', $bukuTamu->kontak) }}"
                    class="w-full border rounded p-2">
            </div>

            <div>
                <label class="block font-medium">Keterangan</label>
                <textarea name="keterangan" class="w-full border rounded p-2">{{ old('keterangan', $bukuTamu->keterangan) }}</textarea>
            </div>
