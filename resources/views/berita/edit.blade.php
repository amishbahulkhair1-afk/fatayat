        ...
        <form action="{{ route('berita.update', $berita->id) }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="col-span-2 space-y-4">
                    ...
                    <div>
                        ...
                        <select name="kategori" class="w-full border rounded p-2">
                            ...
                            @foreach ($kategoriList as $k)
                                <option value="{{ $k }}"
                                    {{ old('kategori', $berita->kategori) == $k ? 'selected' : '' }}>{{ $k }}
                                </option>
                            @endforeach
                        </select>
                        @error('kategori')
                ...
                <div>
                    ...
                    <textarea name="isi_berita" rows="8" class="w-full border rounded p-2">{{ old('isi_berita', $berita->isi_berita) }}</textarea>
                    @error('isi_berita')
                        <p class="text-red-600 text-sm">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="border rounded p-4">
                    ...
                    <label class="flex items-center gap-2 mb-1">
                        <input type="radio" name="status" value="Publik"
                            {{ old('status', $berita->status) == 'Publik' ? 'checked' : '' }}> Publik
                    </label>
                    <label class="flex items-center gap-2 mb-1">
                        <input type="radio" name="status" value="Draft"
                            {{ old('status', $berita->status) == 'Draft' ? 'checked' : '' }}> Draft
                    </label>
                    <label class="flex items-center gap-2">
                        <input type="radio" name="status" value="Dijadwalkan"
                            {{ old('status', $berita->status) == 'Dijadwalkan' ? 'checked' : '' }}> Dijadwalkan
                    </label>
                </div>
            </div>
