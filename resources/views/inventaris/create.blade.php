        ...
        <form action="{{ route('inventaris.store') }}" method="POST"
            ...
            <div class="border-b border-gray-100 pb-4">
                ...
                <p class="text-sm text-gray-500 mt-1">
                    Lengkapi data inventaris dan aset organisasi dengan benar.
                </p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                {{-- Kode Inventaris --}}
                <x-ui.input name="kode_inventaris" label="Kode Inventaris" :value="old('kode_inventaris')" placeholder="INV-001"
                    required />

                {{-- Nama Barang --}}
                <x-ui.input name="nama_barang" label="Nama Barang" :value="old('nama_barang')" placeholder="Contoh: Laptop ASUS"
                    required />
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div x-data="{
                    openKategori: false,
                    labelKategori: '{{ old('kategori') ?: 'Pilih Kategori' }}'
                ...
                {{-- Merk/Tipe --}}
                <x-ui.input name="merk_tipe" label="Merk / Tipe" :value="old('merk_tipe')"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                {{-- Tahun Perolehan --}}
                <x-ui.input name="tahun_perolehan" label="Tahun Perolehan" inputType="number" :value="old('tahun_perolehan')"
                    placeholder="2026" />

                {{-- Kondisi --}}
                <div x-data="{
                    openKondisi: false,
                    labelKondisi: '{{ old('kondisi') ?: 'Pilih Kondisi' }}'
                }" class="relative space-y-2">

                    <label class="block text-sm font-medium text-gray-700">Kondisi</label>

                    <input type="hidden" name="kondisi" value="{{ old('kondisi') }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                ...
                <div x-data="{
                    openLokasi: false,
                    labelLokasi: '{{ old('lokasi_penyimpanan') ?: 'Pilih Lokasi' }}'
                }" class="relative space-y-2">

                    <label class="block text-sm font-medium text-gray-700">Lokasi Penyimpanan</label>

                    <input type="hidden" name="lokasi_penyimpanan" value="{{ old('lokasi_penyimpanan') }}">
                ...
                {{-- Jumlah --}}
                <x-ui.input name="jumlah" label="Jumlah Barang" inputType="number" :value="old('jumlah', 1)" required />

                {{-- Satuan --}}
                <x-ui.input name="satuan" label="Satuan" :value="old('satuan')" placeholder="Unit / Buah / Set" />
            </div>

            {{-- Deskripsi --}}
            <x-ui.textarea name="deskripsi" label="Deskripsi / Keterangan"
                placeholder="Tambahkan keterangan tambahan jika diperlukan...">{{ old('deskripsi') }}</x-ui.textarea>
