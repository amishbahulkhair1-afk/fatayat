        ...
        <form action="{{ route('inventaris.update', $inventaris->id) }}" method="POST"
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="kategori" class="w-full border rounded p-2">
                        ...
                        @foreach ($kategoriList as $k)
                            <option value="{{ $k }}"
                                {{ old('kategori', $inventaris->kategori) == $k ? 'selected' : '' }}>{{ $k }}
                            </option>
                        @endforeach
                    </select>
                    @error('kategori')
                ...
                <div>
                    ...
                    <input type="text" name="merk_tipe" value="{{ old('merk_tipe', $inventaris->merk_tipe) }}"
                        class="w-full border rounded p-2">
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="kondisi" class="w-full border rounded p-2">
                        <option value="Baik" {{ old('kondisi', $inventaris->kondisi) == 'Baik' ? 'selected' : '' }}>
                            Baik</option>
                        <option value="Rusak Ringan"
                            {{ old('kondisi', $inventaris->kondisi) == 'Rusak Ringan' ? 'selected' : '' }}>Rusak Ringan
                        </option>
                        <option value="Rusak Berat"
                            {{ old('kondisi', $inventaris->kondisi) == 'Rusak Berat' ? 'selected' : '' }}>Rusak Berat
                        </option>
                    </select>
                </div>
            </div>
            ...
            <div class="grid grid-cols-3 gap-4">
                <div>
                    ...
                    <select name="lokasi_penyimpanan" class="w-full border rounded p-2">
                        ...
                        @foreach ($lokasiList as $l)
                            <option value="{{ $l }}"
                                {{ old('lokasi_penyimpanan', $inventaris->lokasi_penyimpanan) == $l ? 'selected' : '' }}>
                                {{ $l }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <input type="number" name="jumlah" value="{{ old('jumlah', $inventaris->jumlah) }}"
                        class="w-full border rounded p-2">
                    @error('jumlah')
                        <p class="text-red-600 text-sm">{{ $message }}</p>
                    @enderror
            ...
            <div>
                ...
                <textarea name="deskripsi" class="w-full border rounded p-2">{{ old('deskripsi', $inventaris->deskripsi) }}</textarea>
            </div>
