...
<div class="space-y-6 max-w-5xl mx-auto">
        ...
        <div class="flex items-start gap-3">
            ...
            <div class="min-w-0">
                ...
                <p class="text-xs text-green-800/80 mt-1 leading-relaxed">
                    Perbarui data <span class="font-semibold text-green-900">Lembaga Nahdlatul Ulama</span>
    ...
    <div class="rounded-3xl bg-white border border-gray-200 shadow-sm overflow-hidden">
        ...
        <div class="px-6 py-5 border-b border-gray-200">
            <h2 class="text-lg font-semibold text-gray-900">Form Edit Lembaga</h2>
            <p class="text-sm text-gray-500 mt-1">
                Pastikan informasi lembaga sudah sesuai sebelum menyimpan perubahan data.
            </p>
        </div>

        <form action="{{ route('lembaga.update', $lembaga->id) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')

            <!-- PAC -->
            <div x-data="{
                pacId: '{{ old('pac_id', $lembaga->pac_id) }}',
                namaPac: '{{ old('pac_id', $lembaga->pac_id)
                    ? optional($pac->firstWhere('id', old('pac_id', $lembaga->pac_id)))->nama
                    : 'Pilih PAC' }}'
            }">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <x-ui.date-input name="tanggal_dibentuk" label="Tanggal Dibentuk"
                    :value="old('tanggal_dibentuk', $lembaga->tanggal_dibentuk)" />

                <div x-data="{
                    status: '{{ old('status', $lembaga->status ?? 'Aktif') }}',
                    labelStatus: '{{ old('status', $lembaga->status ?? 'Aktif') }}'
                }">
            ...
            <div class="border-t border-gray-100 pt-6">
                <div class="flex items-center gap-2 mb-4">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-500 mt-0.5">Perbarui pengurus yang menjabat sebagai ketua lembaga.</p>
                    </div>
                </div>

                <div x-data="{
                    ketua: '{{ old('ketua_id', $lembaga->ketua_id) }}',
                    namaKetua: '{{ old('ketua_id', $lembaga->ketua_id)
                        ? optional($pengurus->firstWhere('id', old('ketua_id', $lembaga->ketua_id)))->nama_lengkap
                        : 'Pilih Ketua Lembaga' }}'
                }">
            ...
            <div class="border-t border-gray-100 pt-6">
                <div class="flex items-center gap-2 mb-4">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-500 mt-0.5">Perbarui bidang kegiatan atau deskripsi singkat lembaga.</p>
                    </div>
                </div>

                <textarea name="deskripsi" rows="5"
                    placeholder="Deskripsi singkat atau bidang kegiatan lembaga..."
                    class="w-full rounded-2xl border border-gray-300 bg-white px-4 py-3 text-sm text-gray-900 placeholder-gray-400 shadow-sm focus:border-green-500 focus:ring-4 focus:ring-green-100 transition resize-none">{{ old('deskripsi', $lembaga->deskripsi) }}</textarea>
            </div>
