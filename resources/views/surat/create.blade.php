        ...
        <form action="{{ route('surat.store') }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="border-b border-gray-100 pb-4">
                <h3 class="text-lg font-semibold text-gray-900">
                    Informasi Surat {{ $jenis }}
                </h3>
                <p class="text-sm text-gray-500 mt-1">
                    Lengkapi data surat dengan benar sebelum disimpan.
                </p>
            </div>

            {{-- Nomor Surat --}}
            <x-ui.input name="nomor_surat" label="Nomor Surat" :value="old('nomor_surat')"
                placeholder="Contoh: 001/PAC-FN/VIII/2026" required />

            {{-- Tanggal --}}
            <x-ui.date-input name="tanggal" label="Tanggal {{ $jenis == 'Masuk' ? 'Terima' : 'Kirim' }}"
                :value="old('tanggal')" required />

            {{-- Pengirim / Tujuan --}}
            <x-ui.input name="pengirim_tujuan" label="{{ $jenis == 'Masuk' ? 'Pengirim' : 'Tujuan' }}" :value="old('pengirim_tujuan')"
                placeholder="Masukkan {{ strtolower($jenis == 'Masuk' ? 'pengirim' : 'tujuan') }} surat" required />

            {{-- Perihal --}}
            <x-ui.input name="perihal" label="Perihal" :value="old('perihal')" placeholder="Masukkan perihal surat"
                required />
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                <div x-data="{
                    openJenis: false,
                    labelJenis: '{{ old('jenis_surat') ?: '-- Pilih Jenis Surat --' }}'
                    ...
                    <input type="hidden" name="jenis_surat" value="{{ old('jenis_surat') }}">
                ...
                <div x-data="{
                    openSifat: false,
                    labelSifat: '{{ old('sifat_surat') ?: '-- Pilih Sifat Surat --' }}'
                }" class="relative space-y-2">
                    ...
                    <input type="hidden" name="sifat_surat" value="{{ old('sifat_surat') }}">
